...
namespace GameBasic
{
    public class Game
    {
        ...
        Game(int fieldSize, params int[] values)
        {
        ...
        public void Shift(int value)
        {
            ...
            if (Math.Abs(dx) == 1 ^ Math.Abs(dy) == 1)
            {
                ...
                locations[0] = locations[value];
                locations[value] = tmpPoint;

                int tmpValue = gameField[locations[0].Y, locations[0].X];
                gameField[locations[0].Y, locations[0].X] = gameField[locations[value].Y, locations[value].X];
                gameField[locations[value].Y, locations[value].X] = tmpValue;
            }
            else
